...
class Timer:
    ...
    def report(self):
        ...
        print("TIMING REPORT")
        print("="*80)
        total = sum(v for v in self.times.values() if v > 0)
...
def xcorr_tvt_estimate(
    ...
    return tw_tvt_local[center_idx], float(best_corr)
...
class Timer:
    ...
    def report(self):
        ...
        total = sum(v for v in self.times.values() if v > 0)
        for k, v in sorted(self.times.items(), key=lambda x: -x[1]):
            print(f"{k:<30} {v:10.2f} sec")
        print("-"*70)
        print(f"{'TOTAL':<30} {total:10.2f} sec")
        print("="*70)
...
for wid, h, tw in all_w:
    df_feat, feat_cols = make_feats(h, tw)
...
def make_ds(ids):
    ...
    for wid in ids:
        ...
        Xs.append(zone[feat_cols].to_numpy(np.float32))
        ys.append(zone["tvt_resid"].to_numpy(np.float32))

    X = np.vstack(Xs)
    y = np.concatenate(ys)

    med_ = np.nanmedian(X, axis=0)
    X = fast_nan_fill(X, med_)
    return X, y, feat_cols, med_
